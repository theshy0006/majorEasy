...
class InviteViewModel: NBViewModel {
    var recruitModel = RecruitListModel()
    var pageNum = 1
    var loadMore = false
    var dataSource:[RecruitListItem] = []
    //获取短信验证码（）
    func getSuppliesByParam(
                  success: (()->())?,
                  failure: ((APIError)->())?) {
        recruitModel.getRecruitList(pageNum: pageNum, pageSize: 10, workAreaCode: "", job: "").subscribe(onNext: { [weak self] model in
            if let suc = success {

...
class SecondCarListVC: BaseVC {
    ...
    let viewModel = SecondCarViewModel()
    ...
    lazy var tableView = UITableView(frame: self.view.frame, style: .plain).then {
        ...
        let nib = UINib(nibName: "SecondCarCell", bundle: nil)
        $0.register(nib, forCellReuseIdentifier: "SecondCarCell")
    }
    ...
    func getOrderList() {
        ...
        self.viewModel.getOrderList(type: self.type, success: { [weak self] in
...
//tableView代理实现
extension SecondCarListVC : UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell:SecondCarCell = tableView.dequeueReusableCell(withIdentifier: "SecondCarCell") as! SecondCarCell

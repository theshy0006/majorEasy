...
class SendingViewModel: NBViewModel {
    ...
    func getMySuppliesModel(
                  success: (()->())?,
                  failure: ((APIError)->())?) {
        orderModel.getMySuppliesModel(pageNum: pageNum, pageSize: 10, sortType: "1", appSupplyStatus: "1,2").subscribe(onNext: { [weak self] model in
            if let suc = success {
